javascript:(function(){
  const C = ["Atta","Rice","Oil","Ghee","Dal","Masala","Packaged Foods","Tea","Coffee","Biscuits","Dairy","Sweets","Snacks","Skincare","Cleaning","Protein","Electronics","Home Appliances","Kitchen Appliances"];
  const d = document;
  const c = t => d.createElement(t);
  const q = s => d.querySelector(s);
  const qa = s => Array.from(d.querySelectorAll(s));
  let CA = [], MG = null, SM = "DISC";
  ...
  st.innerHTML = `
    ...
    .z-pin{margin-bottom:10px}
    .z-pin label{font-size:10px;font-weight:700;color:#7b1fa2;display:block;margin-bottom:3px}
    .z-pin input{width:100%;box-sizing:border-box;padding:6px 8px;border:1px solid #ce93d8;border-radius:6px;font-size:12px;font-weight:600;color:#4a148c;outline:none}
    .z-pin input:focus{border-color:#7b1fa2;box-shadow:0 0 0 2px #e1bee7}
    .z-pin .z-pin-ok{display:block;width:100%;margin-top:4px;padding:5px;background:#7b1fa2;color:#fff;border:none;border-radius:6px;font-size:11px;font-weight:700;cursor:pointer}
    .z-pin .z-pin-ok:hover{background:#4a148c}
    .z-pin .z-pin-status{font-size:9px;color:#888;margin-top:2px;display:block}
  `;
  ...
  const x = c("div");
  x.className = "z-head-btn z-close";
  x.innerText = "✕";
  x.onclick = () => dk.remove();
  dk.appendChild(x);

  const mn = c("div");
  mn.className = "z-head-btn z-min";
  mn.innerText = "−";
  mn.onclick = () => { dk.classList.toggle("min"); mn.innerText = dk.classList.contains("min") ? "+" : "−"; };
  dk.appendChild(mn);

  const stb = c("div");
  stb.className = "z-sort-pill";
  stb.innerText = "% Off ⬇";
  stb.onclick = () => { SM = SM === "DISC" ? "PRICE" : "DISC"; SORT(); };
  dk.appendChild(stb);
  ...
  // --- PINCODE SECTION ---
  const pinDiv = c("div");
  pinDiv.className = "z-pin";
  const pinLabel = c("label");
  ...
  const pinBtn = c("button");
  pinBtn.className = "z-pin-ok";
  pinBtn.innerText = "Set Pincode";
  const pinStatus = c("span");
  pinStatus.className = "z-pin-status";
  pinStatus.innerText = pinInput.value ? "Saved: " + pinInput.value : "";
  pinDiv.appendChild(pinBtn);
  pinDiv.appendChild(pinStatus);

  pinBtn.onclick = async () => {
    const pc = pinInput.value.trim();
    if (!/^\d{5,6}$/.test(pc)) { pinStatus.innerText = "Enter valid pincode"; return; }
    localStorage.setItem("z_pin", pc);
    pinStatus.innerText = "Applying...";
    try {
      await applyPincode(pc);
      pinStatus.innerText = "Set: " + pc;
    } catch (e) {
      pinStatus.innerText = "Saved (apply on search)";
    }
  };

  dk.appendChild(pinDiv);
  // --- END PINCODE SECTION ---

  d.body.appendChild(dk);
  d.body.appendChild(ld);

  const w = ms => new Promise(r => setTimeout(r, ms));
  const msg = t => ld.innerText = t;

  const TT = o => {
    let t = d.getElementById("zt");
    if (o) {
      if (!t) { t = c("style"); t.id = "zt"; t.innerHTML = "img{visibility:hidden!important}*{transition:none!important;animation:none!important}"; d.head.appendChild(t); }
    } else if (t) t.remove();
  };
  ...
  const SORT = () => {
    if (!MG || !CA.length) return;
    if (SM === "DISC") { CA.sort((a, b) => b.d - a.d); stb.innerText = "% Off ⬇"; }
    else { CA.sort((a, b) => a.p - b.p); stb.innerText = "Price ⬆"; }
    MG.innerHTML = "";
    const f = d.createDocumentFragment();
    CA.forEach(x => f.appendChild(x.e));
    MG.appendChild(f);
    window.scrollTo({ top: MG.getBoundingClientRect().top + window.scrollY - 130, behavior: "auto" });
  };

  const PROC = async () => {
    msg("Sorting...");
    await w(50);
    const al = qa("a").filter(e => e.textContent.includes("₹"));
    if (!al.length) throw "No items";
    const vt = new Map;
    al.forEach(l => {
      let p = l.parentElement;
      for (let i = 0; i < 4; i++) { if (p) { vt.set(p, (vt.get(p) || 0) + 1); p = p.parentElement; } }
    });
    const sv = [...vt.entries()].sort((a, b) => b[1] - a[1]);
    MG = sv[0][0];
    const tg = "z-tag";
    let pr = [];
    const raw = MG ? Array.from(MG.querySelectorAll("a")) : al;
    for (let i = 0; i < raw.length; i++) {
      if ((i & 63) === 0) await w(0);
      const el = raw[i];
      const tx = el.innerHTML.replace(/<[^>]+>/g, ' ').replace(/\s+/g, ' ').toUpperCase();
      const isBad = tx.includes("BANNER") || tx.includes("NOTIFY") || tx.includes("OUT OF STOCK");
      if (tx.includes("₹") && !isBad) {
        ...
        matches.forEach(m => {
          ...
          if (val > 5 && val < 30000 && !ctx.includes("MONTH") && !ctx.includes("EMI")) nums.push(val);
        });
        if (nums.length > 0) { p = Math.min(...nums.slice(0, 2)); }
        const pctMatch = tx.match(/(\d+)\s*%\s*OFF/);
        const saveMatch = tx.match(/SAVE\s*₹\s*([\d,.]+)/);
        if (pctMatch) { disc = parseFloat(pctMatch[1]); }
        else if (saveMatch && p !== 999999) { const save = parseFloat(saveMatch[1].replace(/[^\d.]/g, "")); disc = (save / (p + save)) * 100; }
        else if (nums.length >= 2) { const mrp = Math.max(nums[0], nums[1]); const sp = Math.min(nums[0], nums[1]); if (mrp > sp && mrp <= sp * 10) disc = ((mrp - sp) / mrp) * 100; }

        const ot = el.querySelector("." + tg);
        if (ot) ot.remove();
        if (disc > 0) {
          const t = c("div");
          t.innerHTML = Math.round(disc) + "%<div style='font-size:9px'>OFF</div>";
          t.className = tg;
          t.style.cssText = "position:absolute;top:10px;left:10px;background:#720e9e;color:#fff;font-weight:700;font-size:13px;padding:4px 6px;text-align:center;border-radius:4px;z-index:10";
          el.style.position = "relative";
          el.appendChild(t);
        }
        const sT = a => { a.setAttribute("target", "_blank"); a.setAttribute("rel", "noopener noreferrer"); };
        el.tagName === "A" ? sT(el) : el.querySelectorAll("a").forEach(sT);
        pr.push({ e: el, d: disc, p: p });
      }
    }
    if (!pr.length) { alert("No items found"); MG.innerHTML = ""; }
    else { CA = pr; SM = "DISC"; stb.style.display = "block"; SORT(); }
    TT(0);
  };

  const SCROLL_AND_LOAD = async () => {
    window.scrollTo(0, 0);
    TT(1);
    msg("Loading...");
    let lc = 0, u = 0;
    while (u < 2) {
      const it = qa("a").filter(e => e.textContent.includes("₹"));
      if (it.length) { it[it.length - 1].scrollIntoView({ block: "start" }); await w(1500); window.scrollBy(0, -150); }
      else { window.scrollBy(0, 500); }
      await w(2000);
      const nc = qa("a").length;
      if (nc === lc) u++; else { lc = nc; u = 0; }
    }
  };

  const AUTO = async (cat, b) => {
    MG = null; CA = [];
    ...
    try {
      ...
      if (pc && /^\d{5,6}$/.test(pc)) {
        ...
        try { await applyPincode(pc); } catch (e) { /* continue even if pincode apply fails */ }
        await w(1000);
      }
  ...
  const MAN = async (b) => {
    ...
    ld.style.display = "none";
    b.innerText = "⚡ Sort Page";
  };

  const mb = c("button");
  mb.className = "z-btn man";
  mb.innerText = "⚡ Sort Page";
  mb.onclick = () => MAN(mb);
  dk.appendChild(mb);

  const lst = c("div");
  lst.className = "z-list";
  C.forEach(x => {
    const b = c("button");
    b.className = "z-btn";
    b.innerText = x;
